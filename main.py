...
def get_args():
    ...
    parser.add_argument("-num_iterations", help="Number of FastRelax rounds to run (at least 3 is recommended)", default=4)
...
def main(peptide_sequence, output_dir, prefix, num_iterations):
    ...
    for template in tqdm(structural_templates):
        if template != f'{REP_PRED_DIR}/templates/1B0G_reordered_const_cart_relaxed_0001.pdb': continue
        print("threading")
    ...
    with open(f"{output_dir}/pred_dscore.csv", "w") as csvfile:
        ...
        for template, pred_dscore in predicted_dscore_dict.items():
            ...
            pdb_file_pdbid = os.path.splitext(pdb_file_base)[0].split("_")[0]
            writer.writerow([peptide_sequence, pdb_file_pdbid, pred_dscore])
