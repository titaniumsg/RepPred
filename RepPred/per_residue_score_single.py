...
def scoring(model, name, outputfile):
	...
	decoy = pyrosetta.pose_from_pdb(model)

	p1 = decoy.pdb_info().pdb2pose('B', 1)
	pO = decoy.pdb_info().pdb2pose('B', 9)

	score_data = []
	score = scorefxn(decoy)
	energies = decoy.energies()
	pepScore = sum([energies.residue_total_energy(i) for i in range(p1, pO+1)])
	...
	data = [name, str(score), str(pepScore)]
...
def feature_headers(end_res):
	outData = [['target_on_template','totalScore','peptideScore']]
...
rundir='./run-dir/relaxed_structures/best_score'
print(target +'_on_'+template +'.pdb')
scoring(rundir+'/'+target +'_on_'+template +'.pdb', f"{target}_on_{template}", f"scores/score_{target}_on_{template}.csv")
